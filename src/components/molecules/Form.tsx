        ...
        <main className="w-full grid gap-6 pb-10 md:pb-0">
            ...
            <InputField className="w-full flex flex-col gap-2">
                ...
                <div className="w-full relative">
                    <input type="number" placeholder="Enter your height" className="w-full h-12 pl-4 pr-8 text-zinc-400 outline-none transition-all duration-200 focus:border-primary border border-zinc-400 bg-transparent" value={heightValue} onChange={(event: React.ChangeEvent<HTMLInputElement>) => setHeightValue(event.target.value)} />
            ...
            <InputField className="w-full flex flex-col gap-2">
                ...
                <div className="w-full relative">
                    <input type="number" placeholder="Enter your weight" className="w-full h-12 pl-4 pr-8 text-zinc-400 outline-none transition-all duration-200 focus:border-primary border border-zinc-400 bg-transparent" value={weightValue} onChange={(event: React.ChangeEvent<HTMLInputElement>) => setWeightValue(event.target.value)} />
                    <Text as="span" className="absolute text-zinc-200 font-bold top-3 right-4">kg</Text>
                </div>
            </InputField>

            <div className="w-full mt-4">
                <Button onClick={calculateBmi} type="button" className="px-6 py-3 bg-primary text-zinc-900 text-sm uppercase font-semibold">Calculate Now</Button>
            </div>

            {
                bmiValue && <div className="w-full flex flex-col p-4 bg-zinc-700">
                    ...
                    <Text as="p" className="text-primary">{bmiMessage}</Text>
                </div>
            }
